...
class ProjectedDiscriminator(nn.Module):
    ...
    def train(self, mode: bool = True):
        self.heads.requires_grad_(mode)
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor, phase="D") -> torch.Tensor:
        ...
        features_tmp = self.dino(x)

        if phase == "D":
            features = {}
            for k, _ in self.heads.items():
                features[k] = features_tmp[k].detach().clone().requires_grad_(True)
        else:
