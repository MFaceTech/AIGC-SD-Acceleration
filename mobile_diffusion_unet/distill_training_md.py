...
from mdUnet import UNet2DMobileModel
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="distillation student weight"
    )
...
def main():
    ...
    if args.student_unet_weight is not None:
        unet = UNet2DMobileModel.from_pretrained(args.student_unet_weight)
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                for i, model in enumerate(models):
                    if isinstance(model, UNet2DMobileModel):
                        model.save_pretrained(os.path.join(output_dir, f"unet_{i}"))
    ...
    def cast_hook(unet,dicts,model_type,teacher=False):
        ...
        if teacher:
            ...
            unet.down_blocks[0].register_forward_hook(getActivation(dicts,'d0',True))
            unet.down_blocks[1].register_forward_hook(getActivation(dicts,'d1',True))
            unet.up_blocks[3].register_forward_hook(getActivation(dicts,'u3',False))
        elif model_type == "md":
            unet.downblock1.register_forward_hook(getActivation(dicts, "d0", False))
            unet.downblock2.register_forward_hook(getActivation(dicts, "d1", False))
            unet.upblock2.register_forward_hook(getActivation(dicts, "u1", False))
        else:
            num_blocks= 3 if model_type=="sd_tiny" else 4
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                loss_features=0
                if args.distill_level in ["sd_small", "kd_d0", "kd_u3", "kd_d0u3"]:
                    for i in range(4):
                ...
                elif args.distill_level == "md":
                    loss_features = loss_features + F.mse_loss(KD_teacher['d0'], KD_student['d0'])
                    loss_features = loss_features + F.mse_loss(KD_teacher['d1'], KD_student['d1'])
                    loss_features = loss_features + F.mse_loss(KD_teacher['u3'], KD_student['u1'])
                else:
                    for i in range(2):
                        loss_features=loss_features+F.mse_loss(KD_teacher['d'+str(i)],KD_student['d'+str(i)])
                ...
                #                 print(colored(f"{name} grad is None", "red"))
                #             else:
                #                 # print(colored(f"{param.grad.shape}", "white"))
                #                 pass
                #         else:
                #             print(colored(f"{name} not required_grad", "green"))
            ...
            logs = {"step_loss": loss.detach().item(), 
                    ...
                    "task_loss": loss_task.detach().item(),
                    "KD_loss": loss_KD.detach().item(),
                    "feature_loss": loss_features.detach().item()
                    }
